...
class Auditor(BaseAgent):
    ...
    def invoke(self, state: GraphState) -> GraphState:
        ...
        self.logger.info(f"[Auditor] Verifying last step: {last_finished_step.step.description}")
        ...
        return self._verify_last_step(last_finished_step, finished_steps[:-1], failed_steps, state)
    ...
    def _verify_last_step(
        ...
        """
        previous_text = ", ".join([step.step.description for step in previous_steps]) if previous_steps else "none"
        step_output = last_step.output or "No output recorded."

        prompt = self._build_verification_prompt(last_step.step.description, previous_text, step_output)
        ...
        try:
            ...
            if response.reason or response.guidance:
                self.logger.error(f"[Auditor] Step failed: {last_step.step.description}. Reason: {response.reason}, Guidance: {response.guidance}")
                failed_steps.append(FailedStep(step=last_step.step, reason=response.reason, guidance=response.guidance))

        except Exception as e:
            self.logger.error(f"[Auditor] Exception during verification: {e}")
            failed_steps.append(FailedStep(step=last_step.step, reason="Auditor exception", guidance=str(e)))

        state["failed_steps"] = failed_steps
        state["next_node"] = Node.PLANNER_AGENT
        return state

    def _build_verification_prompt(self, step_description: str, previous_steps_text: str, step_output: str) -> str:
        ...
        return AuditorPrompts.VERIFICATION.value.format(step_description=step_description, previous_steps_text=previous_steps_text, step_output=step_output)

...
def run():
    project_root = questionary.path(
        ...
        validate=lambda p: True if os.path.exists(p) and os.path.isdir(p) else "Directory does not exist."
    ).ask()

    add_guidelines = questionary.confirm(
        "Do you want to add guideline files? Otherwise agent will suggest some files.", default=False
    ).ask()
    guideline_files = []
    ...
    if add_guidelines:
        while True:
            path = questionary.path(
                ...
                validate=lambda p: True if not p or os.path.isfile(p) else "Must be a valid filepath"
            ).unsafe_ask()
    ...
    if configure_advanced:
        temp_input = questionary.text(
            ...
            validate=lambda val: True if val == "" or (val.replace('.', '', 1).isdigit() and 0.0 <= float(val) <= 1.0) else "Must be a number between 0.0 and 1.0",
        ).unsafe_ask()
        ...
        tokens_input = questionary.text(
            ...
            validate=lambda val: True if val == "" or val.isdigit() else "Must be a valid integer"
        ).unsafe_ask()
        ...
        timeout_input = questionary.text(
            ...
            validate=lambda val: True if val == "" or val.replace('.', '', 1).isdigit() else "Must be a valid number"
        ).unsafe_ask()

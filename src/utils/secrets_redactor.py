...
class SecretsRedactor:
    ...
    def scan_text_for_secrets(cls, text: str) -> Set[str]:
        ...
        potential_secrets = [secret["secret_value"] for secret in secrets_collection.scan_text(text) if secret["is_secret"]]
        valid_potential_secrets = [secret for secret in potential_secrets if secret is not None]
